...
((Get-Content -path "$($sourcedir)FileSystemUtilTest.h.in" -Raw) -replace '@CMAKE_CURRENT_BINARY_DIR@', $builddir.replace("\", "\\")) | Set-Content -Path "$($sourcedir)FileSystemUtilTest.h"
